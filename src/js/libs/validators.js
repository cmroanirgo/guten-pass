...
(function(){ 
	...
	function trimGutenbergPreamble(text) {
		...
		log("cleaning up gutenberg text...")
		const maxlen = 1024*1500;
		if (text.length>maxlen) {
			log('text has been limited to ' + maxlen + ' chars')
	...
	function multilingualValidator(text) {
		// from https://stackoverflow.com/a/22075070/125525
		log("cleaning up multilingual...")
	...
	module.exports =  {
		...
		gutenberg: trimGutenbergPreamble,

		play: function (text) { // a Play, these generally have lots of UPPERCASE: character names and directions.
			text = trimGutenbergPreamble(text);
			log("cleaning up play...")
			text = text.replace(/^\s+[a-z]+\.\s/ig, ' ') // Characters. eg "   King. What bloody man is that? he can report," ==>   " What bloody man is that? he can report,"
				.replace(/^\w[A-Z]+\w/g, ' ') 	// eg All UPPERCASE. eg "SCENE II." ==> "."  ==> (the "." gets removed later)
												//   eg. "LADY STUTFIELD.  ..." ==> "  STUTFIELD." ==> "   ."
			return text;
		},
		klingon: function(text) {
			// trim out all invalid chars
			log("cleaning up klingon...")
